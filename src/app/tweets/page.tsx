...
export default async function TweetsPage() {
  ...
  const pageDescription = stats
    ? `Browse ${stats.totalTweets.toLocaleString()} tweets from the 2010–2025 archive.`
    : "Browse Elon Musk tweets from the 2010–2025 archive.";

  const jsonLd = [
    generateWebPageSchema({
      title: "Tweets — Elon Musk Archive",
      description: pageDescription,
      url: "/tweets",
      dateModified: tweets[0]?.createdAt,
      breadcrumbs: [
        { name: "Home", url: "/" },
    ...
    generateItemListSchema({
      ...
      items: tweets.map((tweet) => ({
        name: `Tweet from ${formatDate(tweet.createdAt)}`,
        url: `/tweets/${tweet.tweetId}`,
        description: tweet.fullText.replace(/\s+/g, " ").slice(0, 160),
      })),
    }),
  ];
